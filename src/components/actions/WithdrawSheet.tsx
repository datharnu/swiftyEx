...
export function WithdrawSheet({ wallets, onClose }: { wallets: Wallet[]; onClose: () => void }) {
  const [walletType, setWalletType] = useState<WalletType>(
    wallets[0]?.wallet_type ?? 'usdt',
  )
  const [amount, setAmount] = useState('')
  ...
  const isNaira = walletType === 'naira'
  const isCrypto =
    walletType === 'usdt' || walletType === 'btc' || walletType === 'ethereum'
  ...
  const maxAmount = useMemo(
    () => formatBalance(balance, { isCrypto }),
    [balance, isCrypto],
  )
        ...
        <>
          ...
          <div className="mt-5">
            <input
              ...
              placeholder="0.00"
              className="w-full rounded-xl border px-4 py-3 text-lg font-semibold text-zinc-900"
            />

            <button onClick={handleMax} className="mt-2 text-xs text-blue-600">
              Max: {getWalletSymbol(walletType)}
              {maxAmount}
            </button>
          </div>
          ...
          <input
            ...
            className="mt-4 w-full rounded-xl border px-4 py-3 text-sm text-zinc-900"
          />

          <button
            onClick={handleSubmit}
            disabled={!isValid}
            className="mt-6 w-full rounded-xl bg-zinc-900 py-3 text-white disabled:opacity-40"
          >
            Continue
          </button>
        </>
      ...
      {step === 'processing' && (
        <div className="flex flex-col items-center py-10 text-center">
          ...
          <p className="text-xs text-zinc-400">
            Securing transaction with SwiftyEx
          </p>
        </div>
      )}
